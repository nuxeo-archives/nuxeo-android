...
public class ResponseCacheTableWrapper extends AbstractSQLTableWrapper {
    ...
    protected void addEntry(SQLiteDatabase db, String key,
            ResponseCacheEntry entry) {
        String sql = "INSERT INTO " + TBLNAME + " (" + KEY_COLUMN + ","
                + CTYPE_COLUMN + "," + CDISP_COLUMN + "," + RTYPE_COLUMN + ","
                + RENTITY_COLUMN + ")" + " VALUES ( ?, ?, ?, ?, ?)";

        String[] args = { key, entry.getReponseContentType(),
                entry.getResponseContentDisposition(),
                entry.getRequestMethod() + "", entry.getRequestEntity() };
        execTransactionalSQL(db, sql, args);
    }

    protected void updateEntry(SQLiteDatabase db, String key,
            ResponseCacheEntry entry) {
        String sql = "UPDATE " + TBLNAME + " set " + CTYPE_COLUMN + " = " + "'"
                + entry.getReponseContentType() + "'," + CDISP_COLUMN + " = "
                + "'" + entry.getResponseContentDisposition() + "',"
                + RTYPE_COLUMN + " = " + "'" + entry.getRequestMethod() + "',"
                + RENTITY_COLUMN + " = " + "'" + entry.getRequestEntity() + "'"
    ...
    public void storeCacheEntry(String key, ResponseCacheEntry entry) {
        ...
        String sql = "select count(*) from " + TBLNAME + " where " + KEY_COLUMN
                + "= '" + key + "'";
    ...
    public ResponseCacheEntry getEntry(String key) {
        ...
        String sql = "select * from " + TBLNAME + " where " + KEY_COLUMN
                + "= '" + key + "'";
        ...
        try {
            if (cursor.getCount() > 0 && cursor.moveToFirst()) {
                ...
                ResponseCacheEntry cEntry = new ResponseCacheEntry(cType,
                        cDisp, null, null);
