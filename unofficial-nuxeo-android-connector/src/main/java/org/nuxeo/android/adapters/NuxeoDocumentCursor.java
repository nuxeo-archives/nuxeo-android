...
public class NuxeoDocumentCursor extends AbstractCursor {
    ...
    protected static final String[] FIXED_COLUMNS = { "_ID",
            DocumentAttributeResolver.STATUS, DocumentAttributeResolver.ICONURI };
    ...
    public NuxeoDocumentCursor(Session session, String nxql,
            String[] queryParams, String sortOrder, String schemas,
            int pageSize, UUIDMapper mapper, boolean updatable) {
        if (mapper != null) {
            this.mapper = mapper;
        } else {
        ...
        if (updatable) {
            docList = new LazyUpdatableDocumentsListImpl(session, nxql,
                    queryParams, sortOrder, schemas, pageSize);
        } else {
            docList = new LazyDocumentsListImpl(session, nxql, queryParams,
                    sortOrder, schemas, pageSize);
        }
        registerEventListener();
    }

    public NuxeoDocumentCursor(OperationRequest fetchOperation,
            String pageParameterName, boolean updatable) {
        this.mapper = new UUIDMapper();
        this.updatable = updatable;
        if (updatable) {
    ...
    public String[] getColumnNames() {
        if (columns == null) {
            List<String> cols = new ArrayList<String>(
                    getCurrentPage().get(0).getProperties().getKeys());
    ...
    public short getShort(int column) {
        return getCurrentDocument().getLong(columns[column]).shortValue();
    }
    ...
    public String getString(int column) {
        ...
        if (column < FIXED_COLUMNS.length) {
            return DocumentAttributeResolver.getString(getCurrentDocument(),
                    FIXED_COLUMNS[column]);
        }
