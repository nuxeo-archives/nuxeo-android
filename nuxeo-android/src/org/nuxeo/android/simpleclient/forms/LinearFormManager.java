...
public class LinearFormManager {
    ...
    public static void displayForm(Activity targetActivity,
            LinearLayout currentLayout, Document currentDocument)
            throws JSONException {
        displayForm(targetActivity, currentLayout, currentDocument,
                DEFAULT_FIELDS, true);
    }

    public static void displayForm(Activity targetActivity,
            LinearLayout currentLayout, Document doc, String fieldDef,
            boolean showHeader) throws JSONException {
        displayForm(targetActivity, currentLayout, doc,
                new JSONArray(fieldDef), showHeader);
    }

    public static void displayForm(Activity targetActivity,
            LinearLayout currentLayout, Document doc, JSONArray fieldArray,
            boolean showHeader) throws JSONException {

        final int padding = targetActivity.getResources().getDimensionPixelSize(
                R.dimen.defaultPadding);

        if (showHeader) {
            final TextView hTextView = new TextView(targetActivity);
            String header = "&nbsp;&nbsp;<b><i>State</i></b> : "
                    + doc.getState();
            String size = doc.getProperties().getString("common:size");
            if (size != null && !"null".equals(size)) {
                header += "  &nbsp;&nbsp; <b><i> Size </i></b> : "
                        + Integer.parseInt(size) / 1024 + "KB";
            }
            ...
            if (desc != null && !"null".equals(desc)) {
                desc = "&nbsp;&nbsp;<b><i>Description</i></b> : "
                        + doc.getProperties().getString("dc:description");
        ...
        for (int i = 0; i < fieldArray.length(); i++) {
            ...
            if (value != null) {
                ...
                String txt = "<b><i>" + field.getString("label")
                        + "</i></b> : ";
